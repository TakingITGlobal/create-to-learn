...
import { useUser, getUser, createUser, updateUser } from './db'
...
function useAuthProvider() {
  ...
  const handleAuth = async (user) => {
    ...
    const isNewUser = (await getUser(user.sub)) ? false : true

    // Create the user in the database if they are new
    if (isNewUser) {
      await createUser(user.sub, { email: user.email, community: [], displayName: null, greeting: null, school: null, interests: [], language: null, userProgress: [] })
    }
